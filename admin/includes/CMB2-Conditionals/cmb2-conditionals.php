...
add_action( 'admin_init', 'cmb2_conditionals_hook_data_to_save_filtering', 99999 );
...
function cmb2_conditionals_footer() {
  ...
  if ( ( 'woocommerce_page_' . WFI_TEXTDOMAIN) == $screen->id ) {
...
function cmb2_conditionals_hook_data_to_save_filtering() {
  ...
  foreach ( $cmb2_boxes as $cmb_id => $cmb2_box ) {
    add_action( "cmb2_{$cmb2_box->object_type()}_process_fields_{$cmb_id}", 'cmb2_conditional_filter_data_to_save', 99999, 2 );
  }
}
